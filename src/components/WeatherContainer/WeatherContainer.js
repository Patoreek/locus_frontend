  ...
  useEffect(() => {
    async function getWeather() {
      try {
        ...
        //! FIND A SOLUTION -- To make a moon appear if it is nighttime
  ...
  return (
    <div className={classes.weatherContainer}>
      <div className={classes.selectedDay}>
        ...
        <p className={classes.selectedDay__temp}>
          {selectedDay.temp}°
          <span className={classes.selectedDay__celius}>c</span>
        </p>
      </div>
          ...
          >
            ...
            <div
              className={`${classes.day} ${
                day == selectedDay ? classes.highlight : null
              }`}
            >
              <p
                className={`${classes.day__temp} ${
                  day == selectedDay ? classes.highlightTemp : null
                }`}
              >
                {day.temp}°
              </p>
              <div className={classes.day__weather}>
                ...
                day.weather === 313 ||
                day.weather === 321 ? (
                  <LightRainSVG className={classes.icon} />
                ) : null}
                ...
                day.weather === 312 ||
                day.weather === 314 ? (
                  <HeavyRainSVG className={classes.icon} />
                ) : null}
                ...
                day.weather === 231 ||
                day.weather === 232 ? (
                  <ThunderSVG className={classes.icon} />
                ) : null}

                {/* //* CLOUDY */}
                {day.weather === 803 || day.weather === 804 ? (
                  <CloudySVG className={classes.icon} />
                ) : null}

                {/* //* OVERCAST */}
                {day.weather === 801 || day.weather === 802 ? (
                  <OvercastSVG className={classes.icon} />
                ) : null}

                {/* //* SUNNY */}
                {day.weather === 800 && time == "day" ? (
                  <SunnySVG className={classes.icon} />
                ) : null}

                {/* //* NIGHT */}
                {day.weather === 500 && time == "night" ? (
                  <NightSVG className={classes.icon} />
                ) : null}
              </div>
            </div>
          </div>
