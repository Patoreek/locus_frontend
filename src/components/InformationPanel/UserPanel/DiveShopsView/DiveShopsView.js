...
import AddDiveShop from "../../../Diveshop/Forms/AddDiveShop/AddDiveShop";
import EditDiveShop from "../../../Diveshop/Forms/EditDiveShop/EditDiveShop";
import DeleteDiveShop from "../../../Diveshop/Forms/DeleteDiveShop/DeleteDiveShop";

import DiveShopListingPanel from "../../../Diveshop/DiveshopListingPanel/DiveshopListingPanel";
    ...
    <div className={classes.diveShopsView}>
      {!addDiveShopModal && !editDiveShopModal && !deleteDiveShopModal && (
        <div className={classes.diveShopsHome}>
          <span
            onClick={() => setDiveShopAdmin(false)}
          ...
          <p className={classes.instructions}>
            This is the admin section for Dive shops. Simply press anywhere on
            the map to add a new dive shop. To edit the site, simple press on
            the marker. Inside the edit form, there is a link to delete the shop
            if needed.
          </p>
          ...
          <div className={classes.diveShopUserList}>
            <div className={classes.diveShopUserList__titleContainer}>
              <h3 className={classes.header}>
                {" "}
                {account.username}'s Added Shops · {shops.length} shops
              </h3>
            </div>
            {shops.map((shop) => (
              <DiveShopListingPanel
                shop={shop}
                edit={true}
                setEditDiveShopModal={setEditDiveShopModal}
              />
            ))}
          </div>
        </div>
      ...
      {addDiveShopModal && <AddDiveShop />}
      {editDiveShopModal && <EditDiveShop />}
      {deleteDiveShopModal && <DeleteDiveShop />}
    </div>
  );
};
